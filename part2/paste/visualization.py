...
def stack_slices_pairwise(slices, pis, output_params=False):
    ...
    if not output_params:
        S1, S2 ,affine= generalized_procrustes_analysis(slices[0].obsm['spatial'], slices[1].obsm['spatial'], pis[0])
    else:
    ...
    for i in range(1, len(slices) - 1):
        if not output_params:
            x, y ,affine= generalized_procrustes_analysis(new_coor[i], slices[i+1].obsm['spatial'], pis[i])
        else:
...
def stack_slices_center(center_slice, slices, pis, output_params=False):
    ...
    for i in range(len(slices)):
        if not output_params:
            c, y , affine= generalized_procrustes_analysis(center_slice.obsm['spatial'], slices[i].obsm['spatial'], pis[i])
        else:
...
def generalized_procrustes_analysis(X, Y, pi):
    ...
    #tY = np.matrix(tY)
    #R = np.matrix(R)
    affineR1 = np.matrix([[1,0,-tX[0]],[0,1,-tX[1]],[0,0,1]])
    affineR21 = np.matrix([[R[0,0],R[0,1],0],[R[1,0],R[1,1],0],[0,0,1]])
    affineR22 = np.matrix([[1,0,-tY[0]],[0,1,-tY[1]],[0,0,1]])
    affineR2 = affineR1.I.dot(affineR21).dot(affineR22)
    #print(affineR2.tolist())
    #print(R)
    return X,Y,affineR2
